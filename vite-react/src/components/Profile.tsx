...
export default function Profile() {    
    ...
    const [update_profile] = useApolloMutation<ProfiledData, ProfileVariables>(UPDATE_PROFILE, {
        onCompleted: (data: any) => {
            setProfileMsg(data.update_profile.message);
            setUserpicture(data.updat_profile.userpic);
    ...
    const [upload_picture] = useApolloMutation<UploadData, UploadVariables>(UPLOAD_PICTURE, {
        onCompleted: (data: any) => {
            setProfileMsg(data.upload_picture.message);
            let userpic: string = `http://127.0.0.1:5000/static/users/${data.upload_picture.userpic}`;
            setTimeout(() => { 
                setProfileMsg(''); 
  ...
  const changePicture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const [activate_mfa] = useApolloMutation<MfaActivationData, MfaActivationVariables>(ACTIVATE_MFA, {
        onCompleted: (data: any) => {
            setProfileMsg(data.activate_mfa.message);
            if (data.activate_mfa.qrcodeurl === null) {
                let qrcode: string = `http://127.0.0.1:5000/static/ images/qrcode.png`;
                setQrcodeurl(qrcode);
            } else {
                console.log(data.activate_mfa.qrcodeurl);
                setQrcodeurl(data.activate_mfa.qrcodeurl);
            }
            setTimeout(() => { setProfileMsg(''); }, 3000);
        },
        onError: (err: any) => {
    ...
    const [change_password] = useApolloMutation<PasswordData, PasswordVariables>(CHANGE_PASSWORD, {
        onCompleted: (data: any) => {
            setProfileMsg(data.change_password.message);
            setTimeout(() => { setProfileMsg(''); }, 3000);
        },
        onError: (err: any) => {
            setProfileMsg(err.message);
