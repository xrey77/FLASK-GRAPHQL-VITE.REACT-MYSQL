...
export default function Mfa() {
  ...
  const [verify_otp] = useApolloMutation<OtpVerificationData, OtpVerificationVariables>(VERIFY_OTP, {
      onCompleted: (data: any) => {
          setMessage(data.verify_otp.message);
          sessionStorage.setItem("USERNAME", data.verify_otp.username);            
          window.setTimeout(() => {
            setMessage('');
